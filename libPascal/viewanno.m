function viewanno(imgset, annoPath, imgPath)
...
% load image set
[ids,gt]=textread(imgset,'%s %d');

for i=1:length(ids)
    
    % read annotation
    rec=PASreadrecord(sprintf('%s/%s.xml', annoPath,ids{i}));
    
    % read image
    I=imread(sprintf('%s/%s.jpg', imgPath,ids{i}));
