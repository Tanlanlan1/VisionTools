			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
    ...
    /* begin{sangdonp} */
    /* --------------- */
    size_t numDesiredThreads = 12;
    omp_set_dynamic(0);                     /* disable dynamic teams */
    omp_set_num_threads(numDesiredThreads); /* override env var OMP_NUM_THREADS */
    
    /* allocate memory */
    SVECTOR** fydelta_par = (SVECTOR**)my_malloc(sizeof(SVECTOR*)*n);
    double* rhs_i_par = (double*)my_malloc(sizeof(double)*n);
    double* rt_viol_par = (double*)my_malloc(sizeof(double)*n); 
    double* rt_psi_par = (double*)my_malloc(sizeof(double)*n);
    long* argmax_count_par = (long*)my_malloc(sizeof(long)*n);
    /* free later!! */
    ...
    for(i=0; i<n; i++) 
    {
        find_most_violated_constraint(&fydelta_par[i], &rhs_i_par[i], &ex[i], fycache[i], n,
				      sm, sparm, &rt_viol_par[i], &rt_psi_par[i], &argmax_count_par[i]);   
    }
    /* ------------- */
    /* end{sangdonp} */
    /* ------------- */
	...
	for(i=0; i<n; i++) {
      ...
      fydelta = fydelta_par[i];
      rhs_i = rhs_i_par[i];
      rt_viol = rt_viol_par[i];
      rt_psi = rt_psi_par[i];
      argmax_count = argmax_count_par[i];
