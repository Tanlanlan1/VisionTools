...
/*
  mwSize labelstdims[2] = {1, 1};
  const char *labelstfield_names[] = {"c", "uv", "wh", "ds", "s", "dudv", "wh_cc", "parts"};
  */
  ...
  /*
  if (! patterns_array ||
      ! mxIsStruct(patterns_array)) {
    mexErrMsgTxt("SPARM.PATTERNS must be a struct array") ;
  }
   **/
...
/*
  if (! labels_array ||
      ! mxIsStruct(labels_array) ||
      ! mxGetNumberOfElements(labels_array) == numExamples) {
    mexErrMsgTxt("SPARM.LABELS must be a struct array "
                 "with the same number of elements of "
                 "SPARM.PATTERNS") ;
  }
*/
  ...
  /*
  for (ei = 0 ; ei < numExamples ; ++ ei) {
    sample.examples[ei].x.mex = mxGetField(patterns_array, ei, "feat") ;
    
    sample.examples[ei].y.mex = mxCreateStructArray(2, labelstdims, 8, labelstfield_names);
    mexPrintf("%f %f\n", sample.examples[ei].x.mex, sample.examples[ei].y.mex);
    while(1);
    
    mxSetField(sample.examples[ei].y.mex, 0, "c", mxGetField(labels_array, ei, "c"));
    mxSetField(sample.examples[ei].y.mex, 0, "uv", mxGetField(labels_array, ei, "uv"));
    mxSetField(sample.examples[ei].y.mex, 0, "wh", mxGetField(labels_array, ei, "wh"));
    mxSetField(sample.examples[ei].y.mex, 0, "ds", mxGetField(labels_array, ei, "ds"));
    mxSetField(sample.examples[ei].y.mex, 0, "s", mxGetField(labels_array, ei, "s"));
    mxSetField(sample.examples[ei].y.mex, 0, "dudv", mxGetField(labels_array, ei, "dudv"));
    mxSetField(sample.examples[ei].y.mex, 0, "wh_cc", mxGetField(labels_array, ei, "wh_cc"));
    mxSetField(sample.examples[ei].y.mex, 0, "parts", mxGetField(labels_array, ei, "parts"));
