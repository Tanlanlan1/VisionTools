...
{    
  ...
  fn_array = mxGetField(sparm->mex, 0, "constraintFn") ;  
  if (! fn_array) {
      /* use the mex funciton of constraintCB */
      /*status = constrintCBFnc((mxArray*) sparm->mex, model_array, x.mex, y.mex, &ybar.mex); /* param, model, xi, yi, ybar */
      
      if (status)
          mexErrMsgTxt("Errors on the constrintCBFnc\n");
  }
  else
  {
      if (! mxGetClassID(fn_array) == mxFUNCTION_CLASS) {
      ...
      mxArray* param = mxCreateStructArray(2, dims, nFieldName, fieldnames);
            
      for(i=0; i<nFieldName; i++)
        mxSetField(param, 0, fieldnames[i], mxGetField((mxArray*) sparm->mex, 0, fieldnames[i]));
