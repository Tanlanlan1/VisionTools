function newimg = perform_image_resize(img,p1_ori,q1_ori,r1)
...
p = size(img,1);
q = size(img,2);
[Y,X] = meshgrid( double(0:q-1)/double(q-1), double(0:p-1)/double(p-1) );
[YI,XI] = meshgrid( double(0:q1-1)/double(q1-1), double(0:p1-1)/double(p1-1) );
newimg = myinterp2( Y,X, img, YI,XI ,'cubic');